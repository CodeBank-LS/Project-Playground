...
int main(){
    ...
    Meal* m7 = new Meal(4,"Fried Fries 管他肥不肥我就爱吃薯条", 7.9);
    Meal* m4 = new Meal(5,"Salad 仙女沙拉", 9.9);
    Meal* m5 = new Meal(6,"Sandwich 社畜加班三明治", 10);
    Meal* m6 = new Meal(7,"Soft Drink 快乐肥宅水", 3);
    ...
    cout<<"Are you ready to order? (Y/N)"<<endl;
    char choice1;
    cin>>choice1;
    //构建一个order
    Order* yourOrder;
    ...
    if(choice1=='Y'||choice1=='y'){
        ...
        if(tableID==1){
            ...
            yourOrder->chooseTable(t1);//add the table to the order
        }
        else if(tableID==2){
            t2->set_booked();
            yourOrder->chooseTable(t2);
        }
        else if(tableID==3){
            t3->set_booked();
            yourOrder->chooseTable(t3);
        }
        else if(tableID==4){
            t4->set_booked();
            yourOrder->chooseTable(t4);
        }
        else if(tableID==5){
            t5->set_booked();
            yourOrder->chooseTable(t5);
        }
        else{
